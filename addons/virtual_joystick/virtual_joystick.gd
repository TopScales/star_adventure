...
var _event_left: InputEventAction
var _event_right: InputEventAction
var _event_up: InputEventAction
var _event_down: InputEventAction
...
func _update_joystick(touch_position: Vector2) -> void:
	...
	if use_input_actions:
		...
		if output.x >= 0 and Input.is_action_pressed(action_left):
			_event_left.pressed = false
			_event_left.strength = 0.0
			Input.parse_input_event(_event_left)
		if output.x <= 0 and Input.is_action_pressed(action_right):
			_event_right.pressed = false
			_event_right.strength = 0.0
			Input.parse_input_event(_event_right)
		if output.y >= 0 and Input.is_action_pressed(action_up):
			_event_up.pressed = false
			_event_up.strength = 0.0
			Input.parse_input_event(_event_up)
		if output.y <= 0 and Input.is_action_pressed(action_down):
			_event_down.pressed = false
			_event_down.strength = 0.0
			Input.parse_input_event(_event_down)
		# Press actions
		if output.x < 0:
			_event_left.pressed = true
			_event_left.strength = -output.x
			Input.parse_input_event(_event_left)
		if output.x > 0:
			_event_right.pressed = true
			_event_right.strength = output.x
			Input.parse_input_event(_event_right)
		if output.y < 0:
			_event_up.pressed = true
			_event_up.strength = -output.y
			Input.parse_input_event(_event_up)
		if output.y > 0:
			_event_down.pressed = true
			_event_down.strength = output.y
			Input.parse_input_event(_event_down)
...
func __create_events() -> void:
	_event_left = InputEventAction.new()
	_event_left.event_index = 0
	_event_left.action = action_left
	_event_right = InputEventAction.new()
	_event_right.event_index = 0
	_event_right.action = action_right
	_event_up = InputEventAction.new()
	_event_up.event_index = 0
	_event_up.action = action_up
	_event_down = InputEventAction.new()
	_event_down.event_index = 0
	_event_down.action = action_down
